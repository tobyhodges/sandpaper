...
#> ℹ Lesson bootstrapped in 3.107268 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01301312 secs
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 3.14 seconds
tik <- Sys.time()
...
#> [md-outputs (root-commit) 257f061] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/RtmpbSPUVK/REMOTE-1e585b11c2e9
...
#> From /tmp/RtmpbSPUVK/REMOTE-1e585b11c2e9
...
#>   /tmp/RtmpbSPUVK/file1e5823fe5b1c/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 257f061)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 257f061 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) a6a0693] Initializing gh-pages branch
...
#>   /tmp/RtmpbSPUVK/file1e5823fe5b1c/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at a6a0693)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at a6a0693 Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 0.35 seconds
tik <- Sys.time()
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 12.93 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpbSPUVK/file1e5823fe5b1c/lesson-example/site/built", "/tmp/RtmpbSPUVK/file1e5823fe5b1c/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpbSPUVK/file1e5823fe5b1c/lesson-example/site/docs", "/tmp/RtmpbSPUVK/file1e5823fe5b1c/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/RtmpbSPUVK/file1e5823fe5b1c/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/RtmpbSPUVK/REMOTE-1e585b11c2e9)
#> /tmp/RtmpbSPUVK/REMOTE-1e585b11c2e9
