...
#> ℹ Lesson bootstrapped in 3.639399 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01376557 secs
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 3.68 seconds
tik <- Sys.time()
...
#> [md-outputs (root-commit) 95edbf3] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/Rtmp2vyL1A/REMOTE-1cade7c285a
...
#> From /tmp/Rtmp2vyL1A/REMOTE-1cade7c285a
...
#>   /tmp/Rtmp2vyL1A/file1cad7394da3f/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 95edbf3)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 95edbf3 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) d57310a] Initializing gh-pages branch
...
#>   /tmp/Rtmp2vyL1A/file1cad7394da3f/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at d57310a)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at d57310a Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 0.5 seconds
tik <- Sys.time()
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 13.71 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmp2vyL1A/file1cad7394da3f/lesson-example/site/built", "/tmp/Rtmp2vyL1A/file1cad7394da3f/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmp2vyL1A/file1cad7394da3f/lesson-example/site/docs", "/tmp/Rtmp2vyL1A/file1cad7394da3f/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/Rtmp2vyL1A/file1cad7394da3f/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/Rtmp2vyL1A/REMOTE-1cade7c285a)
#> /tmp/Rtmp2vyL1A/REMOTE-1cade7c285a
